...
class NL2SQLConfig:
    ...
    def _load_config(self):
        ...
        self.model_name = st.secrets["GROQ_MODEL_NAME"]
...
class GroqNL2SQL:
    ...
    def __init__(self, config: NL2SQLConfig):
        ...
        # OpenAI client will be initialized on-demand for regeneration
        self.openai_client = None
        
    def _initialize_groq_client(self):
        """Initialize OpenAI client with Groq endpoint and Langfuse integration"""
        try:
    ...
    def _initialize_openai_client(self):
        """Initialize OpenAI client for regeneration (on-demand)"""
        try:
            if not self.config.openai_api_key:
                raise ValueError("OpenAI API key not provided for regeneration")
                
            self.openai_client = OpenAI(api_key=self.config.openai_api_key)
            logger.info("✅ OpenAI client initialized successfully for regeneration")
    ...
    def check_query_relevance(self, natural_language_query: str) -> Tuple[bool, str]:
        ...
        logger.info(f"🔍 Checking query relevance for: '{natural_language_query}'")
        
        prompt = f"""
Classify if the user query is about business data or not.

Only mark the greeting messages as NOT_RELEVANT, rest all will be RELEVANT.
...
"""
        ...
        try:
            ...
            logger.info(f"📊 Query relevance check result: {relevance_status}")
...
"""
    ...
    def regenerate_sql_after_error(self, natural_language_query: str, failed_sql: str,
        ...
        """Regenerate SQL query after database execution error using OpenAI"""
        ...
        if not self.openai_client:
            try:
                ...
                error_msg = f"Failed to initialize OpenAI client for regeneration: {str(e)}"
                logger.error(f"❌ {error_msg}")
                return "", error_msg
...
"""
            logger.info("📖 Including conversation context in regeneration")
            
        # Example query to guide the model
        example_query = """
...
10. Only return one final SQL query, nothing else
...
"""
        ...
        try:
            response = self.openai_client.chat.completions.create(
                model="gpt-4o-mini",   # You can change this to gpt-4o or gpt-4-turbo
                messages=[
    ...
    def _is_safe_query_for_execution(self, sql_query: str) -> Tuple[bool, str]:
        ...
        logger.info(f"🔒 Performing safety check on SQL query")
        
        if not sql_query:
            logger.warning("⚠️ Empty query detected")
            return False, "Empty query"
            
        query_upper = sql_query.upper().strip()
        
        if not (query_upper.startswith('SELECT') or query_upper.startswith('WITH')):
            logger.warning("⚠️ Query doesn't start with SELECT or WITH")
    ...
    def generate_description(self, query_result: List[Dict], original_query: str, conversation_context: str = "") -> str:
        ...
        logger.info(f"📝 Generating description for {len(query_result)} query results")
        
        if not query_result:
            description = "No data found matching your query criteria. This could be due to specific filters applied or no records matching the given time period."
            logger.info("📝 Generated description for empty results")
            return description
...
"""
            logger.info("📖 Including conversation context in description generation")
...
You are a professional business data analyst. Based on the provided sales data, give a detailed description of the results. Only provide the analysis of the data, no other information."""
        ...
        try:
            logger.info("📡 Sending description generation request to Groq API...")
            response = self.groq_client.chat.completions.create(
                model=self.model_name,
                messages=[
            ...
            fallback_description = f"Query executed successfully. {result_summary} The data shows various business metrics that can help in making informed decisions for your organization."
            logger.info("📝 Using fallback description")
            return fallback_description

class SmartNL2SQLProcessor:
    """Smart processor with enhanced graph generation, query relevance checking, follow-up context, and Langfuse integration"""
    
    def __init__(self, config_file: str = 'config.ini'):
        logger.info("🚀 Initializing SmartNL2SQLProcessor...")
        ...
        logger.info("✅ SmartNL2SQLProcessor initialized successfully")
    ...
    def process_query_with_smart_visualization(self, natural_language_query: str, conversation_context: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            sql_query, generation_error = self.groq_nl2sql.generate_sql(
            ...
            is_safe, safety_message = self.groq_nl2sql._is_safe_query_for_execution(sql_query)
            ...
            for attempt in range(max_retries + 1):  # 0, 1, 2, ..., 20 (total 21 attempts)
                ...
                if execution_error is None:
                    ...
                    logger.error(f"❌ FAILED on attempt {attempt + 1}")
                    logger.error(f"❌ ERROR: {execution_error}")
                    ...
                    if attempt >= max_retries:
                        logger.error(f"❌ GIVING UP after {max_retries + 1} attempts")
                        result['error'] = f"Query failed after {max_retries + 1} attempts. Final error: {execution_error}"
                        return result
                        
                    # Try to regenerate the SQL using OpenAI
                    ...
                    is_safe, safety_message = self.groq_nl2sql._is_safe_query_for_execution(current_sql)
                    if not is_safe:
            ...
            logger.info(f"📋 STEP 6: Generating visualization...")
            if execution_results:
                ...
                if graph_file:
                    ...
                    logger.warning("⚠️ No suitable visualization could be generated for this data")
                    
            # Step 7: Generate detailed description with context
            logger.info(f"📋 STEP 7: Generating description...")
            description = self.groq_nl2sql.generate_description(
...
def main():
    ...
    logger.info(f"\n{'='*80}")
    logger.info("🚀 SMART NL2SQL PROCESSOR WITH ENHANCED RETRY MECHANISM")
    logger.info(f"{'='*80}")
    ...
    for i, query in enumerate(comprehensive_test_queries, 1):
        ...
        if not result.get('is_relevant', True):
            ...
            if result.get('graph_file'):
                logger.info(f"📈 GRAPH GENERATED: {result['graph_file']}")
                logger.info(f"📈 Graph Type: {result['graph_type']}")
            else:
                logger.info(f"📈 NO GRAPH GENERATED")
    ...
    logger.info(f"\n{'='*80}")
    logger.info("📊 COMPREHENSIVE RESULTS SUMMARY WITH RETRY STATISTICS")
    logger.info(f"{'='*80}")
    ...
    logger.info(f"🔒 Blocked for security: {len(blocked_queries)}")
    logger.info(f"📊 Visualizations generated: {len(graphs_generated)}")
    
    if irrelevant_queries:
        logger.info(f"\n🚫 IRRELEVANT QUERIES:")
        for r in irrelevant_queries:
            logger.info(f"  • '{r['original_query']}'")
            
    if retry_queries:
        logger.info(f"\n🔄 QUERIES THAT REQUIRED RETRIES:")
        for r in retry_queries:
            logger.info(f"  • '{r['original_query']}' - {r['retry_attempts']} attempts")
    ...
    logger.info(f"\n✅ PROCESSING COMPLETE - Check logs directory for detailed logs")
